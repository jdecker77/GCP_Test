...
def build_fpn_keypoint_graph(rois, feature_maps,
    ...
    x = KL.TimeDistributed(
        KL.Lambda(lambda z: tf.compat.v1.image.resize_bilinear(z, [28, 28])),name="mrcnn_keypoint_mask_upsample_1")(x)

    #shape: batch_size, num_roi, 56, 56, num_keypoint
    x = KL.TimeDistributed(
        KL.Lambda(lambda z: tf.compat.v1.image.resize_bilinear(z, [56, 56])),name="mrcnn_keypoint_mask_upsample_2")(x)
...
class MaskRCNN():
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        """
        # assert self.mode == "trainxing", "Create model in training mode."

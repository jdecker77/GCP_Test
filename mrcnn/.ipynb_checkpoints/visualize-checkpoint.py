...
#     print('Drawing Masks')
    ...
    for verts in contours:
        # Subtract the padding and flip (y, x) to (x, y)
        verts = np.fliplr(verts) - 1
        p = Polygon(verts, facecolor="none", edgecolor=color,alpha=.5,linewidth=2,linestyle='dashed')
        ax.add_patch(p)
...
def DrawAnnotations(image, class_ids, class_names, bbox=None, masks=None, keypoints=None, skeleton=None, scores=None, figsize=[18,18],fp="",saveImage=False,raw=None):
    ...
    if saveImage and fp != "":
        ...
        try:
            plt.ioff()
#             Image.fromarray(image).save(fp)
#             i = Image.fromarray(image)
#             ax.add_image(raw)
            plt.savefig(fp, transparent=True)
            plt.close()
        except Exception as e:
            print('Error saving image to',fp,e)
    else:
        pass
        ax.imshow(image)
